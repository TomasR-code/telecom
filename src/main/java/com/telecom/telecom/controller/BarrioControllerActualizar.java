...
@RestController
public class BarrioControllerActualizar {
	...
	public Optional<Barrio> buscarBarrio(@PathVariable Long id) {
		Optional<Barrio> barrioBuscado = barrioService.buscarBarrio(id);
	...
	public ResponseEntity<?> actualizarBarrio(@RequestBody Barrio ba) {
		...
		if (barrio != null) {
			...
			barrioGuardado.setNombre("roberto");
			barrioGuardado = barrioService.guardarBarrio(barrioGuardado);
			return new ResponseEntity<>(barrioGuardado, HttpStatus.OK);
		} else {
